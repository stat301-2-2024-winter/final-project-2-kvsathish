...
# save fit
save(rf_tune, file = here("results/rf_tune.rda"))


## Tuned model with offensive variant trees recipe ----

# specify model for tuning
rf_mod_off <- rand_forest(
  trees = 500,
  min_n = tune(),
  mtry = tune()
) |> 
...
extract_parameter_set_dials(rf_mod_off)


# define workflow
rf_mod_wflow_off <- workflow() |> 
  add_model(rf_mod_off) |>  
  add_recipe(off_trees)

# hyperparam values for tuning
rf_params_off <- extract_parameter_set_dials(rf_mod_off) |>
  update(mtry = mtry(range = c(1, 8)))

rf_grid_off <- grid_regular(rf_params_off, levels = 5)

rf_tune_off <- rf_mod_wflow_off |> 
  tune_grid(
    bball_folds, 
    grid = rf_grid_off,
    control = control_grid(save_workflow = TRUE)
...
# save fit
save(rf_tune_off, file = here("results/rf_tune_off.rda"))